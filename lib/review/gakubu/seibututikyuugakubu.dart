...
final index = algolia.instance.index('seibutu');

class seibututikyuu extends StatefulWidget {
  const seibututikyuu({Key? key}) : super(key: key);

  @override
  State<seibututikyuu> createState() => _seibututikyuuState();
}

class _seibututikyuuState extends State<seibututikyuu> {
  final _queryController = TextEditingController();
  String gakubu = 'seibutu';
...
//大学のアカウント以外は非表示にする
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
          elevation: 0,
          title: !_searchBoolean ? Text('生物地球学部') : _searchTextField(),
