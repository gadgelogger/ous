...
class keiei extends StatefulWidget {
  const keiei({Key? key}) : super(key: key);

  @override
  _keieiState createState() => _keieiState();
}

class _keieiState extends State<keiei> {
  ...
  Future<List<AlgoliaObjectSnapshot>> _search(String query) async {
    final result =
    await _algolia.instance.index('keiei').search(query).getObjects();
...
//algolia
            ...
            : StreamBuilder(
          stream:
          FirebaseFirestore.instance.collection('keiei').snapshots(),
...
class _DetailsScreenState extends State<DetailsScreen> {
  ...
  Future<void> addToFavorites(String documentId, String userId) async {
    ...
    final originalDocRef = FirebaseFirestore.instance.collection('keiei').doc(documentId);
