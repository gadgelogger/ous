...
class kougakubu extends StatefulWidget {
  const kougakubu({Key? key}) : super(key: key);

  @override
  _kougakubuState createState() => _kougakubuState();
}

class _kougakubuState extends State<kougakubu> {
  ...
  Future<List<AlgoliaObjectSnapshot>> _search(String query) async {
    final result =
    await _algolia.instance.index('kougakubu').search(query).getObjects();
...
//algolia
            ...
            : StreamBuilder(
          stream:
          FirebaseFirestore.instance.collection('kougakubu').snapshots(),
...
class _DetailsScreenState extends State<DetailsScreen> {
  ...
  Future<void> addToFavorites(String documentId, String userId) async {
    ...
    final originalDocRef = FirebaseFirestore.instance.collection('kougakubu').doc(documentId);
